...
const BIGINT *bigint_const[CONST_COUNT] = {
	[ZERO_VAL] 	= &(BIGINT) {
		.buf = (BIGINT_BUFFER[1]) {  0 },
		.size = sizeof(BIGINT_DIGIT)
	},
	[TEN_VAL] 	= &(BIGINT) {
		.buf = (BIGINT_BUFFER[1]) { 10 },
		.size = sizeof(BIGINT_DIGIT)
	},
};

void _bigint_dbgprint(BIGINT *a) {
	const int cap = BIGINT_CAP(a);

	for(int i = 0; i < cap; i++) {
		fprintf(__DEBUG_OUT, "%02x ", a->buf[i]);
	}
	fprintf(__DEBUG_OUT, "\n");
}
...
/* Ripple carry (?)
 *
 * Carry the value of 'n' to position 'pos' in BIGINT 'big'.
 * If the value of 'n' does not fit in a single digit of
 * base BIGINT_BASE the carry is executed recursively until
 * a value is left that does fit in a single digit of base
 * BIGINT_BASE or until BIGINT overflows.
 *
 * Returns 0 on success, a nonzero value on error.
 *
 * If an error occurs the appropriate BIGINT flag will be set. */

/* TODO Optimise this */
static inline uint32_t bigint_carry(BIGINT *a, size_t pos, uintmax_t carry) {
	const size_t 	cap = BIGINT_CAP(a);
	uintmax_t 	accum;
...
/* Shift left */
/* TODO implement */
BIGINT *bigint_shl(BIGINT *t, size_t n) {
	((void) t);
	((void) n);
...
/* Shift right */
/* TODO implement */
BIGINT *bigint_shr(BIGINT *t, size_t n) {
	((void) t);
	((void) n);
...
int bigint_btoi(const BIGINT *b) {
	int res = 0;
...
#if 0
/* FIXME: Support hexadecimal */
BIGINT *bigint_sset(BIGINT *big, const char *str) {
	big->flag |= _bigint_stob_base10(big->buf, big->size, str);
	return big;
}
#endif

/* TODO: description of what this does, and why it exists */
/* FIXME [update] this can be removed, it is no longer needed, or used. */
static BIGINT *bigint_buf_add_part(BIGINT *dest, const BIGINT_BUFFER *a,
		const BIGINT_BUFFER *b, const size_t alen, const size_t blen)
{
	assert(BIGINT_CAP(dest) >= alen);
